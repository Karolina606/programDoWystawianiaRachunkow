...
public class Rachunek implements Serializable {
	...
	public void zwiekszIloscZakupu(Produkt produkt, int ilosc) {
		for(Zakup zakup: koszyk){
			if (zakup.getNazwa() == produkt.getNazwa()){
				zakup.zwiekszIlosc(ilosc);
			}
		}
	...
	public void dodajZakup(Zakup zakup){
		for(Zakup zakupWKoszyku: koszyk){
			if (zakupWKoszyku.equals(zakup)){
				zakupWKoszyku.zwiekszIlosc(zakup.getLiczba());
				return;
			}
		}

		this.koszyk.add(zakup);
	}
	...
	public void setKoszyk(List<Zakup> koszyk) {
		this.koszyk = koszyk;
	}
}